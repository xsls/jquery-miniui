...
<% 		
    ...
    try{
        ...
        String json = cn.hemw.miniui.JSON.Encode(result);
        response.reset();
...
{ 		
    ...
    HashMap result = new cn.hemw.miniui.TestDB().SearchEmployees(key, pageIndex, pageSize, sortField, sortOrder);
    String json = cn.hemw.miniui.JSON.Encode(result);
    response.getWriter().write(json);
}
...
public void SaveEmployees(HttpServletRequest request, HttpServletResponse response) throws Exception
{
    ...
    ArrayList rows = (ArrayList)cn.hemw.miniui.JSON.Decode(json);
    ...
    for(int i=0,l=rows.size(); i<l; i++){
        ...
        if(state.equals("added") || id.equals(""))	//新增：id为空，或_state为added
        {
            ...
            new cn.hemw.miniui.TestDB().InsertEmployee(row);
        }
        else if (state.equals("removed") || state.equals("deleted"))
        {
            new cn.hemw.miniui.TestDB().DeleteEmployee(id);
        }
        else if (state.equals("modified") || state.equals(""))	//更新：_state为空，或modified
        {
            new cn.hemw.miniui.TestDB().UpdateEmployee(row);
        }
    }
}
...
public void GetEmployee(HttpServletRequest request, HttpServletResponse response) throws Exception
{
    ...
    HashMap user = new cn.hemw.miniui.TestDB().GetEmployee(id);
    String json = cn.hemw.miniui.JSON.Encode(user);
    response.getWriter().write(json);
}
public void GetDepartments(HttpServletRequest request, HttpServletResponse response) throws Exception
{
    ArrayList data = new cn.hemw.miniui.TestDB().GetDepartments();
...
{
    ArrayList data = new cn.hemw.miniui.TestDB().GetPositions();
    String json = cn.hemw.miniui.JSON.Encode(data);
    response.getWriter().write(json);
}
public void GetEducationals(HttpServletRequest request, HttpServletResponse response) throws Exception
{
    ArrayList data = new cn.hemw.miniui.TestDB().GetEducationals();
    String json = cn.hemw.miniui.JSON.Encode(data);
    response.getWriter().write(json);
}
...
{
    String id = request.getParameter("id");
    ArrayList data = new cn.hemw.miniui.TestDB().GetPositionsByDepartmenId(id);
    String json = cn.hemw.miniui.JSON.Encode(data);
    response.getWriter().write(json);
}
...
public void GetDepartmentEmployees(HttpServletRequest request, HttpServletResponse response) throws Exception
{
    ...
    HashMap result = new cn.hemw.miniui.TestDB().GetDepartmentEmployees(dept_id, pageIndex, pageSize);
    String json = cn.hemw.miniui.JSON.Encode(result);
    response.getWriter().write(json);
}
...
public void SaveDepartment(HttpServletRequest request, HttpServletResponse response) throws Exception
{
    ...
    ArrayList departments = (ArrayList)cn.hemw.miniui.JSON.Decode(departmentsStr);
    
    for(int i=0,l=departments.size(); i<l; i++){
    	HashMap d = (HashMap)departments.get(i);  		
    	new cn.hemw.miniui.TestDB().UpdateDepartment(d);
    }
}
...
public void FilterCountrys(HttpServletRequest request, HttpServletResponse response) throws Exception
{
    ...
    ArrayList data = (ArrayList)cn.hemw.miniui.JSON.Decode(s);
