...
<%@page import="cn.hemw.miniui.TreeUtil"%><%@ page language="java" contentType="text/html; charset=UTF-8"
...
{ 	
    ...
    ArrayList folders = new cn.hemw.miniui.TestDB().DBSelect(sql);
    
    String json = cn.hemw.miniui.JSON.Encode(folders);
...
{ 	
    ...
    for (int i = 0, l = folders.size(); i < l; i++)
    {
        ...
        ArrayList nodes = new cn.hemw.miniui.TestDB().DBSelect(sql2);
...
public void SaveTree(HttpServletRequest request, HttpServletResponse response)throws Exception
{
    ...
    ArrayList tree = (ArrayList)cn.hemw.miniui.JSON.Decode(dataJSON);
    ArrayList removed = (ArrayList)cn.hemw.miniui.JSON.Decode(removedJSON);
    ...
    for(int i=0,l=list.size();i<l;i++){
           ...
           {
                new cn.hemw.miniui.TestDB().InsertNode(node);
           }else{
                new cn.hemw.miniui.TestDB().UpdateTreeNode(node);   
           }
    }
    // Remove Node
    if(removed != null)
    {
           for(int j =0 ,len = removed.size();j<len;j++)
           {
                ...
                new cn.hemw.miniui.TestDB().RemoveNode(removedNode);
           }
    }
}
...
public void FilterTree(HttpServletRequest request, HttpServletResponse response)throws Exception
{
    ...
    ArrayList nodes = new cn.hemw.miniui.TestDB().DBSelect(sql);
  ...
  //返回JSON
    String dataJson = cn.hemw.miniui.JSON.Encode(data);
